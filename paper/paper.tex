...
\usepackage[dvipsnames]{xcolor} % for BrickRed color in abbrevs.tex
\usepackage{hyperref}
...
\title{Reducing finitary inductive-inductive types to inductive types}
...
extrinsic--intrinsic translation, but only for parts (1)--(5), and the
following parts are missing: $(\prop{1})$--$(\prop{5})$. We need
the $(\prop{1})$--$(\prop{5})$ parts to justify the next section.
...
\section{Constructing all IITs from the theory of signatures}
\label{sec:andras}

Show using the CwF model that the existence of the theory of IIT
signatures (Section \ref{sec:theory_of_signatures}) entails the
existance of all IITs.
