...
difference is that we avoid the usage of impredicativity as we don't
require that signatures form a signature algebra. It is also not a
problem for our use case that signatures are not in the lowest
universe.
\begin{definition}[IIT signature]
  ...
  \end{alignat*}
\end{definition}
Note that as $\Sign$ refers to $\SignAlg$, it lives in a higher
universe level than $\SignAlg$. The compatibility condition says that
if we obtain an $M$-context using ${sig}$ in model $M$ and then we
transport it to $N$ using $f$, we get the same $N$-context as directly
applying ${sig}$ to $N$.
...
\begin{definition}[Syntactic signatures]\label{def:syntacticsignature}
  In a model of ETT supporting the theory of ITT signatures, elements
  of $\Con^\I$ are called syntactic signatures. We can turn a
