...
\begin{abstract}
  ...
  where e.g.\ the sort of types is indexed by contexts.  In previous work it was
  shown that all finitary IITs can be constructed from a quotient
  ...
  IIT. In this paper we construct this theory of signatures using only inductive
  types, thereby showing a reduction of all finitary IITs to inductive types.
  We rely on function extensionality and uniqueness of identity proofs. All
  constructions in this paper were formalised in the proof assistant Agda.
\end{abstract}
...
\todo[inline]{add examples where all the different constructors are used}
...
\item
  The term model construction also restrict in a straightforward way. For QIITs,
...
formalisations use UIP and function extensionality, hence by the known
translations between extensional and intensional type
theories\cite{hofmann95extensional,winterhalter2019eliminating}, we can switch
between strict and propositional equalities.
