...
\todo{András: overall todos: introduction, section 2 examples, intro to section 3, future work finish, maybe changing big Sigma notation to (x : A) x B notation (or changing the other way?)}

Most mutual inductive types can be reduced to indexed inductive types
where the index gives the information which sort we meant. An example
is the predicates $\mathsf{isEven}$ and $\mathsf{isOdd}$ defined
mutually by the following constructors.
\begin{alignat*}{5}
...
theory itself, and the main example is still the James Chapman style
\cite{chapman09eatitself} syntax for type theory. A minimal use case
...
The elimination principle for an IIT has two motives and one method
...
As opposed to mutual inductive types, reducing IITs to inductive types
is an open problem. Forsberg \cite{forsberg-phd} showed a reduction
which only supports a simpler, non recursive-recursive elimination
principle (\todo{check}). Hugunin \cite{jasper} reduced an example IIT
to inductive types without UIP, but he also only constructed the
...
\begin{itemize}
  ...
  \item Dependent functions, notated as $(x : A)\ra B$, or as $A \ra B$ when non-dependent.
  ...
  \item Intensional propositional equality as $t = u$, with $\refl : t = t$. In
    this paper, we use extensional notation, and do not write explicit
    transports over equalities. However, we occasionally indicate by
