...
is a quotient inductive-inductive type) to describe a simpler class of
inductive types. Hence we use a Church encoding of the theory of IIT
signatures to define signatures thus avoiding the need for
pre-existing inductive definitions. Another feature of our signatures
is that as they can include types inside the model of ETT (such as
$\N$ in the $\mathsf{isEven}$--$\mathsf{isOdd}$), hence signatures
arex internal to the particular model of ETT.
...
of these type formers through examples after the definitions.
...
\end{definition}
This can be seen as a Church encoding of $\Con$. For our purposes it
is not a problem that signatures don't live at the lowest universe
level.
...
\begin{definition}[A model of ETT supports IITs]\label{sec:hasiits}
  A model of ETT supports IITs if for any signature
  $f:(M:\SignAlg)\ra\Con^M$ there is a $\con_f: (f\,\ADS)^\A$ and an
  $\elim_f : (\gamma^D:(f\,\ADS)^\D\,\con_f)\ra
  (f\,\ADS)^\S\,\con_f\,\gamma^D$.
\end{definition}
...
\end{example}
\begin{remark}
  Note that as we work in a model of ETT, the computation rules that
  are expected from the elimination principle are given up to the
  internal equality type, however by equality reflection we also get
...
In this section we first define what it means that a model of ETT
supports the theory of IIT signatures, then we show that if this is
the case, then it supports all IITs.
