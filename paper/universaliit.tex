...
\item Construction of a morphism $\rec:\mathsf{S}\to M$ for arbitrary $M$, by:
  ...
  \item defining a relation $\blank \sim \blank$ between the well-typed syntax
    ...
    $\GG \sim \model{\GG}$ if and only if $\rec\,\,\GG
...
\begin{alignat*}{5}
  ...
  \end{alignat*}
$\length{\GG}$ denotes the length of a the context $\GG$.
...
following definition:
  ...
  \end{alignat*}
  The idea is that if $\sigma$ is a substitution between contexts $\GG$ and
  $\GD$, then $\keep\,\sigma$ is a substitution between contexts $\GG\ext A[\sigma]$
  and $\GD\ext A$ for any type $A$. This occurs when defining
  $(\Piu\,A\,B)[\sigma]$ as $\Piu\,(A[\sigma])\,(B[\keep \,\sigma])$.




  We can define the identity substitution on a context $\GG$ as
