...
\item Definition of untyped syntax (as a family of inductive datatypes) together
  with typing judgments (as inductive relations on the untyped syntax), and
  construction of a model of the theory of IIT signatures from well-formed
  terms, denoted $\mathsf{S}$.
\item Construction of a morphism $\rec:\mathsf{S}\to M$ for arbitrary $M$, by:
  \begin{enumerate}
  \item defining a relation $\blank \sim \blank$ between the well-typed syntax
    and a given model. The idea is that given a syntactic context $\Gamma$ and a
    semantic context $\model{\Gamma}$ of the model $M$, we have
    $\Gamma \sim \model{\Gamma}$ if and only if $\rec\,\,\Gamma
    = \model{\Gamma}$, and similarly for types, terms, and substitutions;
  ...
  \item Proving the uniqueness of this morphism by showing that any morphism
    $f:\mathsf{S}\to M$ satisfies the relation. For example, for any syntactic context
...
The main interest of $\erru$ is that it behaves like a closed term (which the
theory of signatures lacks), in the sense that it is
invariant under substitution. This makes expected equalities about 
substitution true even in the ill-typed case,
thus reducing the number of hypotheses for the corresponding lemmas
(see Section~\ref{ss:exch-laws}).
\begin{alignat*}{5}
...
\,(A[\sigma])\,(B[\wkO\,\sigma \,\consu\, \varu\,\mathsf{0}])$, and thus we need to define
$\wkO$, the weakening of substitutions.  The basic idea is to increment the de
Bruijn indices of all the variables.  Actually, this is not so simple because of
the $\Piu$ type: we want to define $\wkO\,(\Piu\,A\,B)$ as the $\Pi$ type
...
reason, we need to generalise the weakening as occuring anywhere in the context.
\begin{alignat*}{5}
  & \wk_n && :  \Tyu\ra\Tyu \\
  & \wk_n && :  \Tmu\ra\Tmu \\
  % & \wk && : \N \ra \N\ra\N \\
...
typing. Stating a typing rule for this operation requires weakening at the
middle of a context. We consider pairs of untyped contexts, which should be
thought of as a splitting of a context at some position. The full context is
recovered by merging the two components:
...
\begin{alignat*}{5}
  ...
  \end{alignat*}
$\length{\Gamma}$ denotes the length of a the context $\Gamma$.
...
following definition:
  ...
  & \keep && : \Subu \ra \Subu \\
   &&& := \lambda \sigma. (\varu\, \mathsf{0} \,\consu\, \wkO\,\sigma)
  \end{alignat*}
  The idea is that if $\sigma$ is a substitution between contexts $\Gamma$ and
  $\Delta$, then $\keep\,\sigma$ is a substitution between contexts $\Gamma\ext A[\sigma]$
  and $\Delta\ext A$ for any type $A$. This occurs when defining
  $(\Piu\,A\,B)[\sigma]$ as $\Piu\,(A[\sigma])\,(B[\keep \,\sigma])$.




  We can define the identity substitution on a context $\Gamma$ as
  follows, where $\keep^{\length{\Gamma}}$ is $\keep$ iterated
  $\length{\Gamma}$ times:
\begin{alignat*}{5}
  & \idu && : \Conu \ra \Subu \\
...
\begin{alignat*}{5}
  ...
  & \vzw  && : (\Gamma\vdash)\ra(\Gamma\vdash A)\ra\Gamma \extu  A \vdash \mathsf{0} \invar \wku \, A \\
...
following sense:
\begin{alignat*}{5}
 & \isp{\Conw} && : \isaprop\, (\Gamma\vdash) \\
 & \isp{\Tyw} && : \isaprop\, (\Gamma\vdash A) \\
 & \isp{\wf\Var} && : \isaprop\, (\Gamma\vdash x \invar A) \\
 & \isp{\Tmw} && : \isaprop\, (\Gamma\vdash t \in A) \\
 & \isp{\wf\Sub} && : \isaprop\, (\Gamma\vdash \sigma \Rightarrow \Delta)
\end{alignat*}
...
$\syn\cdot \sim \model\Delta$ we removed.  Our way out here is to state that $\model\sigma$
...
= \model{\Gamma})\times (\rec\,A = _{e\#}\model{A})$.
...
In the following, we abbreviate $\relT{\wf{A}}{\model\Gamma}{\model{A}}$ by
...
\begin{alignat*}{5}
  ...
  \\
  & \emptyw\sim \model{\Gamma} && := \model{\Gamma}=\model{\boldsymbol{\cdot}} \\
 ...
 \model{\epsilon})
  ...
  (e_\Delta : \model{\Delta} = \model{\Gamma} \model{\ext} \model{A})
    ...
    \\
    & && \qquad
     (e_B : \model{B} = _{e_\Delta \#} \model{\wk}\,\model{A}) \times
     % \\
     % & && \qquad
     (\model{t} = _{e_\Delta,e_B \#} \model{\vz})
  \\
  &
  ...
  (e_\Delta : \model{\Delta} = \model{\Gamma} \model{\ext} \model{B})
    ...
    \\
    & && \qquad
     (e_C : \model{C} = _{e_\Delta \#} \model{\wk}\,\model{A}) \times
     \\
     & && \qquad
     (\model{t} = _{e_\Delta,e_C \#} \model{\vs} \, \model{n})
  \\
  & \rlap{(2) Universe} \\[0.5em]
...
%   & {\leq}\boldsymbol{\cdot}  && : \model{\Gamma} \leq \model{\Gamma} \\
...
%   & \model\Gamma \merge (\model\Gamma,{\leq}\boldsymbol{\cdot}) && :=  \model\Gamma \\
...
%   \model\Delta =  {\leq}\boldsymbol{\cdot}\\
...
\begin{alignat*}{5}
  ...
  \\
  & && \qquad
   \wf{{\circ}}\,\wf{\Gamma}\,\wf{\delta}\,\wf{\sigma} \sim \model{\delta}\mathbin{\model{\circ}}\model{\sigma}
  \end{alignat*}
