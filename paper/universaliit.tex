...
Let us mention another possible solution avoiding the recursive-recursive
...
Then, we show that the relation is stable under weakening and substitution.
  ...
  % some redundancies that we could not avoid when defining
  % the relation, for example in the case of the empty substitution.
  Because of this,
  % It is because of the redundancies which we mentionned above
...
semantic counter-parts of the syntax:
  \begin{alignat*}{5}
     & \Sigma\Con{\sim} && :
    (\wf{\Gamma} : \Gamma \vdash ) \ra
...
It is then straightforward to show (without induction) that the first projection of these
